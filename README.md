...
| **FFMQ-15** | 15 | 五因素正念能力 | ⚠️ 题目待 Claude Code 核对 |
| **SCS-SF** | 12 | 自我关怀（6 维度） | ⚠️ 题目待 Claude Code 核对 |

### 可选模块（按主诉触发）

| 量表 | 题数 | 评估 | 触发条件 |
|---|---|---|---|
| **MAIA-2** | 37 | 内感受 / 身体觉察（8 维度） | "和身体断联" |
| **DERS-SF** | 18 | 情绪调节困难（6 维度） | "情绪上头" |
| **PSQI** | 19 | 过去一个月睡眠质量 | "睡眠不好" |
| **ECR-12** | 12 | 依恋焦虑 / 回避 | "亲密关系反复出问题" |

> ⚠️ 标记为"题目待核对"的量表，目前数据文件中题目内容是占位符。计分结构、维度、反向题位置都已经按权威源设置好，Claude Code 接力时只需要把中文题目内容填入即可。详见 `docs/scales-sources.md`。

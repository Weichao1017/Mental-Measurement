  ...
  for (const { scale, result } of results) {
    ...
    for (const w of result.warnings) {
      signals.push({
        ...
        description: `警示题命中："${truncate(w.itemText, 30)}" → 答 ${w.answer}`,
        level: "urgent",
        warning: true,
      });
    }
    ...
    if (!scale.highIsBetter) {
      for (const d of result.dimensions) {
        ...
        const label = d.band?.label ?? "";
        if (level === "extremely_severe") {
          signals.push({
            scaleId: scale.id,
            scaleName: scale.name,
            description: `${d.name}: ${label}`,
            level: "urgent",
          });
...
const SUMMARIES: Record<ClinicalLevel, string> = {
  urgent: "你的评估结果出现了需要立即关注的信号。建议尽快寻求专业精神科或心理科评估，不要独自承担。",
  strong: "你的多项指标命中临床关注切点。建议在近期（一周内）预约精神科医生或临床心理咨询师做正式评估。",
  consult: "你的部分指标提示存在一些中度困扰。建议结合心理咨询 / 正念课程进行支持性介入，并在 1-2 个月后复测观察趋势。",
  self_help: "目前各项指标基本在常规范围内。通过正念练习、自我关怀、规律作息等自助方式维持即可。",
};

const RECOMMENDATIONS: Record<ClinicalLevel, string[]> = {
  urgent: [
